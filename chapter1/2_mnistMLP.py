...
def train(model, train_loader, optimizer, log_interval):
    ...
    for idx, (x_train, y_train) in enumerate(train_loader):
        ...
        if idx % log_interval ==0:
            print(f"Train Epoch : {Epochs}\t [{idx*len(data)} / {len(train_loader.dataset)} ({100.*idx/len(train_loader):.6f}%)]\tTrain Loss : {loss.item():.6f}")
...
for Epoch in range(0, Epochs):
    ...
    print(f"\nEpochs : {Epoch}\tTest_Loss : {test_loss}\tTest_accuracy : {test_accuracy}")
